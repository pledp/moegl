...
impl GraphicsContext {
    ...
    pub fn end_draw(&mut self, render_data: RenderData) {
        self.queue.submit(std::iter::once(render_data.encoder.finish()));
        render_data.output.present();
    }

    pub fn clear(&mut self, render_data: &mut RenderData, color: glm::TVec3<f64>) {
        let mut _render_pass = render_data.encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("Render Pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: &render_data.view,
                resolve_target: None,
